...
struct message_t *buffer_to_message(char *msg_buf, int msg_size){
  ...
  if(msg_size < 8){return NULL;}//2 OP + 2 CT + 4 do result, pois eh o mais pequeno
  ...
  if(msg->opcode != OC_SIZE && msg->opcode != OC_DEL && msg->opcode != OC_UPDATE 
        && msg->opcode != OC_GET && msg->opcode != OC_PUT && msg->opcode != (OC_SIZE+1) && 
	msg->opcode != (OC_DEL+1) && msg->opcode != (OC_UPDATE+1) && msg->opcode != (OC_GET+1) && 
        msg->opcode != (OC_PUT+1) && msg->opcode != OC_RT_ERROR){
    free_message(msg);
    return NULL;
  }
