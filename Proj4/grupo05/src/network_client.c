	...
	*/
	...
	if(msg_resposta->opcode != (OC_SIZE + 1) && msg_resposta->opcode != (OC_DEL + 1) && 
		msg_resposta->opcode != (OC_UPDATE + 1) && msg_resposta->opcode != (OC_GET + 1)
		 && msg_resposta->opcode != (OC_PUT + 1) && msg_resposta->opcode != (OC_HELLO + 1)
		 && msg_resposta->opcode != (OC_SERVER + 1) && msg_resposta->opcode != OC_RT_ERROR){
 	...
 	switch(msg->c_type){
 		...
 		default:
			...
			else if((msg_resposta->opcode == (OC_HELLO + 1)) && (msg_resposta->c_type != CT_RESULT)){
	 			free(message_out); free(message_in); return NULL;
			}
			else if((msg_resposta->opcode == (OC_SERVER + 1)) && (msg_resposta->c_type != CT_RESULT)){
	 			free(message_out); free(message_in); return NULL;
			}
 	}
...
struct message_t *reconnect(struct message_t *message){
	...
	if(rec < 1){
		network_close(server);
		server = network_connect(addressport);
		if(server == NULL) {
